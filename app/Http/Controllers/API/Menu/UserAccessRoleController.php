...
class UserAccessRoleController extends Controller
{
    ...
    {
        try {
            $data = M_MasterUserAccessRole::all();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $request->validate([
                'master_role_id' => [
                    ...
                    function ($attribute, $value, $fail) use ($request) {
                        $exists = DB::table('master_users_access_role')
                            ->where('users_id', $request->user()->id)
                            ->where('master_role_id', $request->master_role_id)
                            ->exists();
            ...
            M_Role::where('id',$request->master_role_id)->firstOrFail();
            ...
            M_MasterUserAccessRole::create($validator);

            DB::commit();
            return response()->json(['message' => 'Users Access Role created successfully', "status" => 200], 200);
        } catch (ModelNotFoundException $e) {
            DB::rollback();
